...
```{r}
colData <- data.frame(condition=factor(c("exposed", "exposed", "control", "control", "exposed", "exposed", "exposed", "control", "control", "control", "control", "exposed", "control", "exposed")), 
...
```{r}
vsd.transpose.df$condition <- c("exposed", "exposed", "control", "control", "exposed", "exposed", "exposed", "control", "control", "control", "control", "exposed", "control", "exposed")
head(vsd.transpose.df)
```
...
Clear environment, and re-read in the count matrix and pull out the 14 libraries

```{r}
colData <- data.frame(condition=factor(c("exposed", "exposed", "control", "control", "exposed", "exposed", "exposed", "control", "control", "control", "control", "exposed", "control", "exposed")), 
                             type=factor(rep("paired-end", 14)),
                             size=factor(c("large", "large", "small", "large", "small", "small", "large", "large", "large", "large", "small", "small", "small", "large")))
rownames(colData) <- colnames(convad)
dds <- DESeqDataSetFromMatrix(countData = convad,
                                     colData = colData, 
...
```
Wrote out table 20240112. Commented out code
...
head(deglistcount)
```
write out genelistcount:
```{r}
#write.table(deglistcount, "../analyses/08-deseq2/DEGlist_control_v_armdrop_7x7_contrastsize_genecounts.tab", sep = "\t", row.names = T, quote = FALSE, col.names = TRUE)
```
wrote out 20240112
...
Annotate heatmap with control or exposed:
```{r}
sample_col <- data.frame(treatment = c("control", "control", "control", "control", "control", "control", "control", "exposed", "exposed", "exposed", "exposed", "exposed", "exposed", "exposed"))
row.names(sample_col) <-colnames(nozerodegheatmap)
sample_col
...
add annotation for star age:
```{r}
sample_col$star.size <- c("juvenile", "adult", "adult", "adult", "adult", "juvenile", "juvenile", "adult", "adult", "juvenile", "juvenile", "adult", "juvenile", "adult")
sample_col
```
specify colors for annotation columns:
```{r}
my_colors <- list(
  treatment = c(control = "#15BED2", exposed = "#E33607"),
  star.size = c(adult = "#AF2CEC", juvenile = "#18E307")
)
```
